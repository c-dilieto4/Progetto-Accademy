...
def process_dialogflow_webhook(req):
    ...
    # 2. ESTRAZIONE DATI GLOBALE (Ad ogni interazione)
    ...
    # 3. ASSEGNAZIONE DEL CODICE TRIAGE (Quando viene stimato il dolore)
    if intent_name == 'AnalisiDolore':
        livello = globals.ultimo_dato_dolore['pain_level']
        globals.dati_paziente['livello_dolore'] = livello
        
        if livello in ["0 Dolore Lieve", "Dolore Lieve", "Lieve"]:
            codice_paziente = "VERDE"
        elif livello in ["1 Dolore Moderato", "Dolore Moderato", "Moderato"]:
            codice_paziente = "ARANCIONE"
        else:
            codice_paziente = "ROSSO"
            
        globals.dati_paziente['codice'] = codice_paziente
        return jsonify({"fulfillmentText": f"Dall'analisi visiva, rilevo un {livello}. Ti ho assegnato il Codice Triage: {codice_paziente}. Confermi i dati per il salvataggio?"})
